...
Handlebars.registerHelper("if_minus", function(num, options) {
	if(num<0){
		return options.fn(this);
	}else{
		return options.inverse(this);
...
/**
 * If Equals test_comp this compare=that
 */
Handlebars.registerHelper('line_color', function(context, options) {
	var colors = options.hash.color.split(",");
	var len = options.data.length;
	if(len > 6){
		if(context < 3)
			return colors[0];
		else if(context > options.data.length-4){
			return colors[2];
		}else{
			return colors[1];
		}
...
Handlebars.registerHelper('isPositive', function(a1,options) {
	if (a1.indexOf("-")>=0) {
		return options.fn(this);
	}
	return options.inverse(this);
...
Handlebars.registerHelper('isRealNum', function(a1,options) {
    if (a1!="-") {
        return options.fn(this);
    }
    return options.inverse(this);
...
Handlebars.registerHelper('isRate', function(a1,options) {
    if (a1=="-"||a1.indexOf("PP")>=0){
       return options.inverse(this);
    }else if (a1.indexOf("比")>=0||a1.indexOf("率")>=0) {
		return options.fn(this);
	}
	return options.inverse(this);
});